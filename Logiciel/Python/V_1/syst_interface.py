...
derniere_valeur_valide_hum = None
...
def genAfficheCRIFA():
    ...
    # connection internet
    initLabel(root, "Status de Connection", 22, "red", syst_config.TKPOS_LEFT, syst_config.TKPOS_BOTTOM) # text, width, couleur, pos x,y
    lblInternet = initLabelValue(root, 6, syst_config.TKPOS_LEFT, syst_config.TKPOS_BOTTOM + syst_config.TKPOS_OFFSET) # width, pos x,y

    # Moyenne temperature
    initLabel(root, "Temp. MOY (°C)", 17, "dark green", syst_config.TKPOS_CENTER, syst_config.TKPOS_TOP) # text, width, couleur, pos x,y
    ...
    #Time
    initLabel(root, "Heure", 5, "lavender", syst_config.TKPOS_RIGHT, syst_config.TKPOS_TOP) # text, width, couleur, pos x,y
    lblTime = initLabelValue(root, 6, syst_config.TKPOS_RIGHT, syst_config.TKPOS_TOP + syst_config.TKPOS_OFFSET) # width, pos x,y
...
def getValues():
    ...
    global derniere_valeur_valide_hum   
    ...
    try:
        if getSensors_atlas.humVal != -1 and getSensors_atlas.humVal != 0:
            derniere_valeur_valide_hum = getSensors_atlas.humVal  # Mettre à jour la dernière valeur valide
            lblHUM.config(text=str(getSensors_atlas.humVal))
        else:
            raise Exception()  # Forcer l'exception si la valeur est invalide
    except:
        if derniere_valeur_valide_hum is not None:
            lblHUM.config(text=str(derniere_valeur_valide_hum))  # Utiliser la dernière valeur valide
        else:
            lblHUM.config(text='---')  # Aucune valeur valide n'a été enregistrée jusqu'à présent
    
    try:
        if syst_config.useSSLWebsockets|syst_config.useUnsecuredTCP|syst_config.useUnsecuredWebsockets:
            lblInternet.config(text='ON')   
        elif syst_config.useUnsecuredUDP:
            lblInternet.config(text='ON')
        else :
    ...
    try:
        if getSensors_atlas.co2Val != -1 and getSensors_atlas.co2Val != 0 and getSensors_atlas.co2Val is not None :
            derniere_valeur_valide_co2 = getSensors_atlas.co2Val  # Mettre à jour la dernière valeur valide
            lblCO2.config(text=str(getSensors_atlas.co2Val))
        else:
            raise Exception()  # Forcer l'exception si la valeur est invalide
    except:
        if derniere_valeur_valide_co2 is not None:
            lblCO2.config(text=str(derniere_valeur_valide_co2))  # Utiliser la dernière valeur valide
        else:
            lblCO2.config(text='---')  # Aucune valeur valide n'a été enregistrée jusqu'à présent

    try:
        lblTime.config(text=str(syst_logique.getTime()))        
    except:
        lblTime.config(text='----')
